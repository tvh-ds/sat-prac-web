/** Print passage + prompt of questions. */
...
for (const q of res.questions) {
  if (!q.sourceModuleName.toLowerCase().includes(modFilter!.toLowerCase())) continue;
  if (q.sourceQuestionNumber < from || q.sourceQuestionNumber > to) continue;
  console.log(`### #${q.sourceQuestionNumber} p${q.pageNumber} passage=${q.passageText ? q.passageText.length + "ch" : "NONE"}`);
  console.log(`PASSAGE: ${(q.passageText ?? "").slice(0, 150)}`);
  console.log(`PROMPT: ${q.prompt.slice(0, 120)}`);
}
